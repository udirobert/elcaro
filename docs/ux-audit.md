...
- **BG3 · WebMCP posture is undefined — closed.** ChatGPT’s in-app browser and Chrome’s WebMCP testing flag landed. `/scan` registers `scan_content`, `load_specimen`, `list_specimens`, `explain_verdict`, and `contrast_intent` via `document.modelContext.registerTool`. The last is the joint review: the agent’s intended action appears next to the injection ([docs/webmcp.md](webmcp.md)). → R9.
...
| ID | Item | Finding | Effort | Principles |
|---|---|---|---|---|
| R9 | **WebMCP on `/scan`** — five `registerTool`s; `contrast_intent` writes the agent’s intended action onto the page. The playground is human-voiced (title + Together rail); tool names stay in `llms.txt` / `/integrate`. [docs/webmcp.md](webmcp.md). | BG3 | M | 8 |
