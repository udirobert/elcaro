...
> Date: 2026-08-28 · Status: audit complete — **all twelve roadmap items are implemented**, and R9 has moved from “tool-declarable form” to a live imperative WebMCP registration on `/scan` (2026-09-01; [docs/webmcp.md](webmcp.md)): R1 (MCP server), R2 (agent-legible layer), R3 (relay contract), R4 (correction surface), R5 (signed verdicts), R6 (reviewer mode), R7 (threshold replay), R8 (content-type-aware notices), R9 (WebMCP tools on `/scan`), R10 (supervision surface — session watch), R11 (warn-salience experiment — executed 2026-08-30: 81 completions across 3 model families, judge-rescored compliance prefix 0/27, sandwich 0/27, suffix 3/27; prefix remains the default, results published in `docs/warn-salience-experiment.md` and on /for-agents), R12 ("designing for agents" page).
...
- **BG3 · WebMCP posture is undefined — closed.** ChatGPT’s in-app browser and Chrome’s WebMCP testing flag landed. `/scan` registers `scan_content`, `load_specimen`, `list_specimens`, and `explain_verdict` via `document.modelContext.registerTool` and drives the same React form a human uses ([docs/webmcp.md](webmcp.md)). → R9.
...
- **BG6 · warn-mode salience was unexamined — now tested.** The middleware's "warn" mode *prepends* its warning, then lets the injected content follow. Elcaro's own taxonomy (`placement_salience`) says document edges carry instruction weight — so prefix is defensible — but the warning then competes with an injection adversarially optimized against exactly such warnings. → R11: dogfood the taxonomy with a prefix/suffix/sandwich experiment. Executed 2026-08-30 (81 completions, 3 model families): compliance prefix 0/27, sandwich 0/27, suffix 3/27. The taxonomy's prediction held — suffix (injection last) was the only position that leaked. Prefix stays the default, now documented as load-bearing (`docs/warn-salience-experiment.md`, /for-agents).
...
| ID | Item | Finding | Effort | Principles |
|---|---|---|---|---|
| R9 | **WebMCP on `/scan`** — imperative `document.modelContext.registerTool` for `scan_content`, `load_specimen`, `list_specimens`, `explain_verdict`. Tools fill the playground; they do not silent-fetch. Spec still a CG draft; ChatGPT in-app browser and Chrome flag are the test surfaces. [docs/webmcp.md](webmcp.md). | BG3 | M | 8 |
...
Sequencing: R1–R4 are small and independent — any order, all before the P1 batch. R8 depends on R3. R9 shipped 2026-09-01 as imperative tools on `/scan`.
